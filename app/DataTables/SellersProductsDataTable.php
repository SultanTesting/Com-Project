...
class SellersProductsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        {
            $pending = ($query->approved == 'pending') ? 'selected' : '';
            $failed  = ($query->approved == 'failed') ? 'selected' : '';
            $published = ($query->approved == 'published') ? 'selected' : '';

            return "<select class='form-control form-select is_approved' data-id='$query->id'>
                <option $pending value='pending'> Pending </option>

                <option $published value='published'> Published </option>
        ...
        {
            if($query->offer_price)
            {
                return "<span class='badge badge-info'>$query->offer_price $</span>";
            }
                return "<span class='badge badge-secondary'>No Offers</span>";
        })
        ...
        ->rawColumns(['action', 'created_at', 'status', 'image', 'approved', 'offer_start_date', 'offer_end_date', 'offer_price', 'product_type'])
        ->setRowId('id');
    }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('price')->width(75)->addClass('font-weight-bold'),
